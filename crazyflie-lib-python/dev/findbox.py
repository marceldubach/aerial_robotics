...
class LoggingExample:
    ...
    def __init__(self, link_uri):
        ...
        self.landing_site = np.array([0.4,0,self.default_height])
    ...
    def fly_square(self, id):
        ...
        self.waypoints = np.array([
                                ...
                                [x_min,y_center,self.default_height]])
        directions = [1,2,1,4,3,2]
        print('Hardcoded directions',directions)
        ...
        # Send position commands
            with PositionHlCommander(scf,default_height=0.5) as pc: #default_height=0.5
                ...
                print('Number of edges found (should be 1 here): ', len(self.edges))
                if (len(self.edges)==1):
                    ...
                    print(self.position)
                    time.sleep(1)
                    #pc.go_to(edge1_x+self.delta, edge1_y, self.default_height,vel)
                    pc.go_to(center_x, center_y, self.default_height, vel)
                    time.sleep(1)
                    orthog_x, orthog_y = self.go_orthogonal(self.edges[0])
                    #pc.go_to(edge1_x+self.delta, edge1_y+self.delta, self.default_height,vel)
                    ...
                    #pc.go_to(edge1_x+self.delta, edge1_y, self.default_height,vel*2)
                    time.sleep(2)
                    #if (self.edge_detection()):
                    #   edge2_x = self.edges[1][0]
                    #   edge2_y = self.edges[1][1]
                    #   self.landing_site = np.array([edge1_x+self.delta, edge2_y-self.delta, self.default_height])
                    #else:
                    #    self.landing_site = np.array([edge1_x+self.delta, edge1_y+self.delta/2, self.default_height])
                    #    print('No second edge found')

                    land_x, land_y = self.find_landing_site(self.edge_detection())
                    self.landing_site[0] = land_x
                    self.landing_site[1] = land_y

                    print('Position before landing', self.position)
                    self.stored_position = []
    ...
    def find_landing_site(self,second_edge_found):
        if (second_edge_found):
            ...
            print('Number of edges found (should be 2 here):',len(self.edges))
    ...
    def edge_detection(self):
        ...
        if (self.ready):
            ...
            z = np.asarray(z)
            print('Number of stored positions: ',len(self.stored_position))
            conv_prod = [np.sum((z[i:i+self.T-1]-self.default_height)*self.kernel) for i in range(z.shape[0]-self.T)]
            #threshold = np.asarray(conv_prod)<-self.conv_thresh
            #threshold = np.hstack([np.asarray([(self.T-1)*[False]]).flatten(),np.asarray(conv_prod)<-self.conv_thresh])

            is_edge = any(np.array(conv_prod)<self.conv_thresh)
            print('Minimum of convulution: ', np.min(conv_prod))
        ...
        for idx in range(len(threshold)-1):
            if (not threshold[idx] and threshold[idx+1]):
                #edge = self.stored_position[idx]
                self.edges.append(self.stored_position[idx])
                print('Edge index: ', idx)
                edge_detected = True
        #self.stored_position = [] # clear stored positions
        """
